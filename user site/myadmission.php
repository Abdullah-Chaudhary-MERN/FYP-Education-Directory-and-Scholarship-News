...
<div>
  ...
  <div class="container row">
    <h1 class="text-center text-dark">Admission in Process</h1>
  <?php
    include("conn.php");
    $sql="SELECT * FROM `addmissionform` WHERE `uid`='$uid' AND `status`=''";
    $res=mysqli_query($conn,$sql);
    foreach($res as $row){
    ?>
<div class="card m-3" style="width: 18rem;">
  <img src="uploadfile/<?php echo $row['student'] ?>" class="card-img-top" alt="...">
  <div class="card-body text-dark">
    <h5 class="card-title"><?php echo $row['name'] ?></h5>
    <p class="card-text"><?php echo $row['cinc'] ?></p>
    <a href="delete.php?did=<?php echo $row ['id'] ?>" class="btn btn-danger">Delete</a>
    <a href="edit.php?eid=<?php echo $row ['id'] ?>"class="btn btn-primary">Edit</a>
  </div>
</div> 
<?php } ?> 
    </div>
<br>
<div class="container row">
    <h1 class="text-center text-dark">Admission Approved</h1>
  <?php
    include("conn.php");
    $sql="SELECT * FROM `addmissionform` WHERE `uid`='$uid' AND `status`='approve'";
    $res=mysqli_query($conn,$sql);
    foreach($res as $row){
    ?>
<div class="card m-3" style="width: 18rem;">
...
<br>
<div class="container row">
    <h1 class="text-center text-dark">Admission Rejected</h1>
  <?php
    include("conn.php");
    $sql="SELECT * FROM `addmissionform` WHERE `uid`='$uid' AND `status`='reject'";
    $res=mysqli_query($conn,$sql);
    foreach($res as $row){
    ?>
<div class="card m-3" style="width: 18rem;">

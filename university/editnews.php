...
<div>
  ...
  <?php
$eid=$_GET['eid'];
$sql="SELECT * FROM `newsreport` WHERE `id`='$eid'";
$result=mysqli_query($conn,$sql);
$row=mysqli_fetch_assoc($result);
?>
  <form class="col-md-6 mx-auto p-3 mt-5 " action="work.php?eid=<?php echo $row['id']?>" method="post" enctype="multipart/form-data" style="background-color: black;opacity: 0.8; margin-top: 120px; ">
   <h1 style="color:white;text-align:center;font-family: Verdana, Geneva, Tahoma, sans-serif;" >News Form</h1>
    ...
    <input type="text" class="form-control" placeholder="Enter News Name " required name="news" value="<?php echo $row['news']?>">
    <label class="form-label"style="color: white;">
    Date
    </label>
    
    <input type="datetime-local" class="form-control" placeholder="Enter Your Clossing Date" required name="date" value="<?php echo $row['date']?>">
    <label class="form-label"style="color: white;">
      Text message
    </label>
    <textarea class="form-control" required name="text"> <?php echo $row['text']?></textarea>
    <label class="form-label" style="color: white;">
      Upload Image
    </label>
    <input type="file" class="form-control" placeholder="Upload Your Image" required name="upload" value="<?php echo $row['upload']?>">
    ...
    <div class="row">
      ...
      <div class="col-3"></div>
      <div class="col-3"></div>
      <div class="col-3"><button type="submit" name="editnews" class=" btn btn-success">send</button></div>
